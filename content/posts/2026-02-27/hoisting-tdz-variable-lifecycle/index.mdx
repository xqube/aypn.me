...
- If it sees `var x = 10;`, it registers `x` and *immediately initializes* its memory to `undefined`.
- If it sees `let y = 20;`, it registers `y`, but leaves the memory address **uninitialized**.
...
2. **Safe Hoisting:** By using `function createPool() {}` instead of `const createPool = () => {}`, the senior developer leverages V8's "Hoisting Phase 1" assignment. `createPool` is fully available in memory the exact millisecond the file begins executing, allowing it to be safely invoked by `initializeDatabase` without rearranging the file structure purely for the engine's sake.
...
Using `let` in a `for (let i = 0; ...)` loop forces V8 to implicitly create a brand new Lexical Environment and re-initialize the variable for *every single iteration*. This safely locks the variable for closures, but heavily punishes V8's New Space Garbage Collector at high throughput.
...
In modern V8 (2026), **there is no execution performance difference between `let` and `const`**. TurboFan (the JIT compiler) analyzes the Data Flow Graph independently of your keywords. If you declare a variable with `let` but never reassign it, TurboFan optimizes it identically to `const`. 
...
1. **Relying on `typeof` for safety checks:**
   In the legacy era, checking if a variable existed using `typeof x === 'undefined'` was safe, even if `x` was never declared. It was the standard way to check for polyfills.
   ```javascript
   // Safe if feature is not declared
   if (typeof window.Promise === 'undefined') { ... } 
   ```
   If a team migrates `Promise` to a `let` block scope, `typeof` no longer protects you. Using `typeof` on a variable stuck in the Temporal Dead Zone will violently crash with a `ReferenceError`. The TDZ actively breaks the `typeof` safety net.
